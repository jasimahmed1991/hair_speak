...
$sql="select sub_categories.*,categories.categories from sub_categories,categories where categories.id=sub_categories.categories_id and categories.status = 1 order by sub_categories.sub_categories asc
";
						 ...
						 <tbody>
							...
							<tr>
							   ...
							   <td><?php echo $row['sub_categories']?></td>
							    <td><img src="<?php echo SUB_CATEGORY_IMAGE_SITE_PATH.$row['image']?>"/></td>
							   <td>
								<?php
								if($row['status']==1){
